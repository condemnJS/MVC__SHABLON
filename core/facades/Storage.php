...
class Storage {
    static public $pathToUpload = APP_DIR . "/public"; // Куда будет загружаться файл по дефолту

    static public function move(string $where, $what) {
        $fileName = $what['name']; // Имя файла
        $fileTmpPath = $what['tmp_name']; // Временное хранилище

        $pathToUpload = 'http:://' . $_SERVER['HTTP_HOST'] . '/public';
        ...
        if(move_uploaded_file($fileTmpPath, $toPath)) {
            return str_replace("\\", '/', $pathToUpload . '/' . $where . '/' . $newFileName);
        } else {
